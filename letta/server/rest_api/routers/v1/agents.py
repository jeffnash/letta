...
async def _import_agent(
    ...
    if override_model_handle:
        llm_config_override, _ = await server.get_llm_config_from_handle_async(actor=actor, handle=override_model_handle)
    else:
...
async def cancel_message(
    ...
    # If run_ids weren't provided, try to resolve "the active run for this agent"
    # from redis (preferred) or fall back to the DB.
    if not run_ids and settings.track_agent_run:
        run_id = None
        try:
            redis_client = await get_redis_client()
    ...
    if clear_pending and not any_cancelled:
        try:
    ...
    if not run_ids and not clear_pending:
        raise NoActiveRunsToCancelError(agent_id=agent_id)

    if failed_to_cancel:
        results["failed_to_cancel"] = failed_to_cancel
...
async def _process_message_background(
    ...
    try:
        ...
        await runs_manager.update_run_by_id_async(
            run_id=run_id,
            update=RunUpdate(status=run_status, stop_reason=stop_reason, completed_at=get_utc_time().replace(tzinfo=None)),
            actor=actor,
        )
        ...
        await runs_manager.update_run_by_id_async(
            run_id=run_id,
            update=RunUpdate(status=RunStatus.failed, stop_reason=StopReasonType.error, metadata={"error": str(e)}, completed_at=get_utc_time().replace(tzinfo=None)),
            actor=actor,
        )
    except Exception as e:
    ...
    def handle_task_completion(t):
        try:
            ...
            async def update_failed_run():
                ...
                await runs_manager.update_run_by_id_async(
                    run_id=run.id,
                    update=RunUpdate(status=RunStatus.failed, stop_reason=StopReasonType.error, metadata={"error": error_str}, completed_at=get_utc_time().replace(tzinfo=None)),
                    actor=actor,
                )
